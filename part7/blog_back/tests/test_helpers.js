...
const notExistingId = async () => {
  const blog = new Blog({ title: "willremovethissoon", url: "-" });
  await blog.save();
...
const getTokenUserId = async () => {
  ...
  const userNameInDB = usersStart[0]?.userName;

  const userInDB = await User.findOne({ userName: userNameInDB });
  ...
  return { token, userId: userInDB._id.toString(), userNameInDB };
};
